...
## 【三千代さんからのご提案】尊厳死（安楽死）制度の検討

回復不能な病や致命的な怪我による耐え難い苦痛がある場合に、本人の明確な意思に基づき、尊厳ある死を選択できる制度の検討を提案します。制度化にあたっては、臓器提供における意思表示カードのような事前登録制度や、本人の意思疎通が不可能になった場合の取り扱いなど、明確かつ厳格な基準を設けることを前提とします。これにより、個人の尊厳を守り、耐え難い苦痛の中で死を待つのではなく、自ら生を全うする選択肢を保障することを目指します。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキGomiGomiださい
