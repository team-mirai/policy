...
チームみらいは非核三原則を堅持し、「核の傘」に依存しない安全保障体制への転換を目指します。核武装の選択肢を明確に否定し、対話による平和構築を主導します。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキ- プできます。
