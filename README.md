...
　なお今後も議論の結果、これらのマニフェストの内容がまだ変わる可能性がある点はご理解いただけますと幸いです。例えば、最近『いどばた』などを通じて頂いた貴重なご意見を踏まえ、マニフェストの改善を検討しています。具体的には、**オンライン投票における脅迫行為への対策（匿名通報制度など）**、**識字困難者も投票しやすいタッチパネル式投票の導入**、さらには**個別の政策に対する実績評価や期待度を投票できる新しい仕組みの導入**などを、具体的な政策として盛り込むことを目指します。私たちは、このように頂いた提案を政策へ反映させることで、マニフェストを成長させていきます。
