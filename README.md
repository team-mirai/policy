...
　現在、匿名ユーザー様との議論を踏まえ、エネルギー政策や経済政策について新たな視点を加えています。エネルギー政策については、発電設備のライフサイクル全体の環境負荷や、事故発生時のリスクと事後処理コストも判断基準に含めるべきというものです。また経済政策については、AIを活用し、自然の生態系のように無駄のない循環型社会を目指すべきという視点を頂きました。このように皆様からのご意見を積極的に取り入れ、育てていくのがこのマニフェストです。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキ_ップできます。
