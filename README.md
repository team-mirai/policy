...
チームみらいは、AIエンジニアの安野たかひろによって設立された政治団体です。テクノロジーで誰も取り取り残さない日本を掲げ、本マニフェストの政策を推進しています。
...
*   **現状の課題**: 育児休業から時短勤務等で復帰後、第二子以降の育児休業を取得した場合、時短復帰後の給与を基準に育児休業給付金が算出されるため、給付額が大幅に減額されます。これは、保育園に意図的に落選して育休を延長するインセンティブとなり、制度の公平性を損なっています。
*   **改善案**: 2回目以降の育児休業給付金について、その時点の給与から計算される給付額と、過去（第一子等）の育休で支給された給付額を比較し、いずれか高い方の金額を支給するようにします。
