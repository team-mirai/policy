...
チームみらいは、AIエンジニアの安野たかひろによって設立された政治団体です。テクノロジーで誰も取り残さない日本を掲げ、本マニフェストの政策を推進しています。また、AI技術を政策推進に活用する際には、生成AIにおけるブラックボックス性（内部処理の不透明さ）を克服し、その判断プロセスの透明性を確保するため、XAI（説明可能なAI）技術の導入を図るなど、倫理的な側面に最大限配慮します。
