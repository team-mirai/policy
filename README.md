...
* **AIの活用と責任についての明確な原則**：本プロジェクトではAI（いどばたAI、チャットBot等）を政策議論の支援として活用していますが、AIは意見や文案を生成する道具であり、いかなる責任も負うことはできません。意思決定、説明責任、倫理的判断はすべて人間の側にあるという原則を厳格に守ります。チームみらいは「責任を取れる人が判断する」ことの重要性を再確認し、この原則に基づいて全ての政策運用を行います。AI開発者は、学習データに用いるコンテンツの権利者から適正な許諾を得ることを原則とし、権利者の意に反したデータ利用を防止する責任を負います。加えて、SNS等のプラットフォーム事業者は、利用者が自身の投稿したコンテンツがAIの学習データとして利用されることを、簡便な方法で拒否（オプトアウト）できる機能を設けることを義務付けます。この義務は、日本国内でサービスを提供する海外事業者にも適用されます。
