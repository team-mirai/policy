...
## 提起された課題：制度の狭間でこぼれ落ちる、複合的な困難を抱える人々への支援

現在の支援制度は、多くの場合、単一の課題（障害、ひとり親家庭など）を想定して作られています。しかし現実には、複数の困難を同時に抱えながらも、懸命に努力し、社会で自立しようとしている人々がいます。

例えば、10代の時の事故で障害を負いながらも、努力の末に国家資格を取得し、二人の子供を育てるシングルマザーがいます。彼女は自身の力で働き、一定の収入を得ています。しかし、その努力の結果として収入が国の定める基準額を超えると、障害基礎年金や児童扶養手当といった、命綱ともいえる支援が次々と打ち切られてしまうのです。

これは「努力した者が罰せられる」という、制度の深刻な矛盾の現れです。

そこで、チームみらいは、こうした複合的な困難を抱える人々を誰一人取り残さないため、以下の実現を目指します。（本件は、ステップ２−５「福祉」の項目で議論を深めます）
...
## 提起された課題：美容医療における医薬品の安全管理強化

### 背景
近年、美容医療の需要が急増し、特にマンジャロ等のホルモン関連注射薬や各種注射剤が流行しています。その一方で、以下の課題が顕在化しています。

*   美容目的で使用される医薬品の安全性に関する長期的な知見が不足している。
*   偽物・並行輸入品などの偽薬の流通が確認され、患者への健康被害が懸念される。
*   医療機関におけるロット管理やトレーサビリティが十分ではなく、薬害発生時に原因特定や被害範囲の把握が難しい。
*   現行のカルテ保存期間（原則5年）では、薬害が顕在化するまでの期間（しばしば10年以上）に対応できない。

このままでは、患者の健康被害が発生した際に適切な救済や原因究明が困難となる恐れがあります。

### 提言
美容医療における医薬品の安全性を確保するため、以下の政策を提言します。

1.  **ロット管理・トレーサビリティの義務化**
    *   美容医療で用いられる医薬品について、ロット番号・製造番号を患者単位で記録する仕組みを法的に義務付ける。
    *   薬局・クリニック間で情報が追跡可能な共通の電子管理システムを整備する。
    *   製造・輸入業者から患者までの流通経路の透明性を確保する。
2.  **偽薬対策の強化**
    *   正規ルート以外の流通や個人輸入による偽薬の排除のため、取り締まりを強化する。
    *   QRコード等で真贋判定が可能なパッケージの採用を促進する。
3.  **医療記録の長期保存**
    *   美容医療において投与された薬剤情報・診療記録を少なくとも10年間保管する制度を検討する。
    *   マイナンバーカード等の公的デジタル基盤に紐付けて記録・保存することで、患者本人が長期にわたり情報を確認できるようにする。
4.  **患者啓発と情報提供**
    *   美容医療を受ける患者に対して、薬剤のリスクや正規品確認方法などの情報提供を義務付ける。
    *   偽薬や不適切な医療行為に関する注意喚起キャンペーンを実施する。

### おわりに
美容医療は自己決定権の尊重のもとに発展してきましたが、その裏側で薬害や不正流通のリスクが顕在化しています。長期的な安全性を担保するため、医薬品の管理体制と記録保存の強化、そしてデジタル技術を活用した新しい仕組みの導入が必要です。国民の健康を守るために、上記施策の速やかな実現を求めます。
