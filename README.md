...
### 現在検討中の政策案（匿名ユーザー様ご提案）
#### 離島・へき地における地域連携型教育の推進
教員不足が課題となる離島やへき地において、地域の多様な人材が学校教育に参加できる仕組みを構築します。地域の専門知識を持つ住民を「地域ティーチャー（仮称）」として認定し、指導要領に沿った形で、教員と連携しながら体験学習などを実施します。これにより教員の負担を軽減しつつ、子どもたちの学びを豊かにします。協力者には謝礼金を支払います。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキ étapesできます。
