...
インバウンド需要の取り込みと国内産業のブランド価値向上のため、政府が認定する新たな品質認証制度を創設します。和菓子、食品、嗜好品、料理店、宿泊施設、工芸品、家電、日用品と、多岐にわたる分野を対象とします。この制度は、品質保持、知名度向上、国内企業の活性化、そして優れた技術の継承を目的とします。

認定基準には、製品・サービスの品質や環境への配慮に加え、「就労環境の良さ」を重要な柱とします。これは、後継者不足に悩む優れた技術を持つ中小企業の事業承継を後押しする狙いもあります。就労環境の評価は、公的データ（労基署など）や民間調査会社（帝国データバンク、転職会議など）の情報を活用し、賃金、労働時間、休暇取得率、ハラスメントの有無などを客観的に判断する仕組みを構築します。集約した就労環境データは、求職者にも提供し、就職のミスマッチ解消を目指します。

認定された事業者は、「エンペラー・ワラント・マーク保持者協会」に加入します。協会は、登録料（分割または一括納付）を財源とし、マークの不正使用の取り締まり、品質管理活動、審査基準の維持・設定を行います。

マークには、登録開始年数と、社会貢献度を示す等級（例：自衛隊の階級章のようなデザイン）を明記します。貢献度は、国内材料の使用、障害者雇用、公的機関への寄付などを基準に評価します。これにより、事業者の長年の実績と継続的な努力が可視化されます。一度認定が取り消された場合、再認定時には最も低い等級から再スタートすることとし、品質と信頼性の維持を促します。
