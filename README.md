...
頂いた変更提案は、チームみらいが責任をもって一つひとつ確認します。重要な議論が他の多くの提案に埋もれてしまわないよう、国政マニフェストとしての優先度やテーマの関連性を考慮し、精査します。これにより、参加者の皆さんが本質的な議論に集中できる環境を整えます。
...
* **悪意のある情報操作への対策**: 誰でも参加できるオープンな性質を悪用した、組織的な情報操作や悪意のある改ざんを防ぐため、すべての変更提案は、その背景や妥当性をチームみらいが精査します。量的な意見だけでなく、質的な議論を重視し、安易な民意形成に流されることなく、最終的な意思決定は人間が責任をもって行います。また、全ての編集履歴は公開され、透明性を担保します。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキEPできます。
