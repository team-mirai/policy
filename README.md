...
チームみらいは、表現の自由を民主主義の最も重要な原則の一つとして捉え、最大限尊重します。いかなる意見も、それ自体を理由に封殺されるべきではありません。

一方で、健全な議論の場を維持するためには、建設的でない言動への対処も必要です。しかし、特定の投稿を「禁止」し、投稿者を排除するという方法は、恣意的に運用されるリスクを常にはらんでいます。権力を持つ者が、自分に都合の悪い意見を封じ込めるためにルールを濫用する可能性を、私たちは常に警戒しなければなりません。

そこで私たちは、安易な「禁止」に頼るのではなく、より丁寧な方法で議論の質を保つことを目指します。例えば、個人攻撃や差別的な発言といった、他者を不当におとしめる投稿に対しては、その投稿の表示優先度を下げるといった技術的な対策を検討します。

その際、多数派の意見によって少数派の意見が不当に扱われることのないよう、公平な判断基準の設計に細心の注意を払います。例えば、特定のグループから特定の意見に対して集中的な通報があった場合に、それを単純な数として扱うのではなく、背景を分析して重み付けを調整するなど、多様な意見が共存できるような仕組みづくりに努めます。
