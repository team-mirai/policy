...
農業について、気候と需要をコンピュータで予測し、地域ごとに生産する作物を最適化し、市場としては安定した価格と供給量を確保、生産者としては安定した収入を確保できるようにします。そして、国民の需要が特に大きい作物、特にコメについては、市場に委ねすぎると農家が稼げないことから作付面積が減少し、結果として今日のようなコメ不足を招きました。市場に委ねすぎず、政府による買い上げを増やします。備蓄の余剰分に関しては、バイオ燃料など、サステナブルな用途への使用を増やすことで政府が一方的に支出する、赤字になることがないようにします。また、新規就農者向けにチャットボットによるサポートを導入し、肥料や農薬、収穫時期についての疑問に迅速に対応できるようにします。インドでは既に実用化されているアイデアです。
