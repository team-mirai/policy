...
りた様より、「デジタル民主主義」に関して、選挙の透明性を高めるための以下のご提案をいただき、反映を検討しています。

*   **公約の比較可視化:** さまざまな政党の主張や公約を、同じ形式で誰もが比較できるようにする。
*   **メディア露出の透明化:** SNS等を含め、特定の候補者や政党への露出の偏りを可視化し、有権者がその状況を理解できるようにする。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキKップできます。
