...
重度障害児（者）とそのご家族が直面する困難を踏まえ、以下の支援策を提案します。

1.  **所得制限の撤廃**: 重度障害児（者）を在宅で介護するご家庭を支えるため、各種障害手当の所得制限を撤廃します。
2.  **障害者手帳の迅速な交付**: 重度の障害がある場合、年齢によることなく速やかに障害者手帳を受け取れるように制度を改善します。
3.  **住環境支援の強化**: 医療的ケア児・重度障害児（者）のいるご家庭が安心して暮らせるよう、公営住宅への入居を優遇するなど、住環境の支援を強化します。
4.  **レスパイトケアの拡充**: 介護者が心身を休められるよう、一時預かり先を拡充し、一度に4泊以上などまとまった日数の利用を可能にします。あわせて、デイケアや入所施設の増設に加え、夜間や泊まりに対応できる在宅レスパイト（訪問看護）を増やし、年間利用時間の上限も緩和します。
5.  **育児休業の延長**: 重度障害児を育てる親が安心して子のケアに専念できるよう、育児休業の期間と手当の支給期間を延長します。

## けいなさんからのご提案（不妊治療と特別養子縁組の支援拡充）

1.  **不妊治療支援の強化**: 質の高い不妊治療の選択肢を増やし、不妊治療や卵子凍結への助成を拡充、手続きを簡素化します。
2.  **特別養子縁組の住居要件の緩和**: 受け入れる子の年齢などに応じ、住居要件（部屋数や面積）を柔軟に運用します。
3.  **養子縁組の費用と質の標準化**: 養親の費用負担に上限を設け、民間団体への助成を増やします。また、研修内容を国が標準化し、質のばらつきをなくします。

## けいなさんからのご提案（医療・技術関連）

1.  **医療デバイスの技術革新と低価格化**: 在宅医療で使う医療機器の小型化、無線化、長時間バッテリー化などを促進し、携帯しやすく安価なデバイスの開発を支援します。
2.  **医薬品の保険適用維持**: 生命の維持に不可欠な医薬品が、安易に保険適用から外されることのないよう、患者の生活への影響を最優先に考え、制度の運用を見直します。
