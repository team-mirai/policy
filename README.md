...
## ステップ１-８．福祉：介護分野のDX（デジタルトランスフォーメーション）推進

介護現場の事務負担を大幅に軽減し、職員が本来のケア業務に集中できる環境を整備するため、テクノロジーを積極的に活用します。

*   **介護情報の連携基盤を構築し、手続きを簡素化します。**
    *   行政が持つ介護度認定情報などをマイナンバーと連携させ、サービス事業者がオンラインで即座に確認できるシステムを整備します。これにより、事業者ごとに行っている保険証の確認作業などの手間をなくします。
*   **介護報酬の請求プロセスをデジタルで完結させます。**
    *   紙や手作業に頼ることが多いサービス実績の報告・請求業務を、デジタルで完結できる一元的なデータ管理システムを行政主導で構築します。
    *   利用者ごとに基本サービスや加算情報をあらかじめ登録。日々の実績管理は、NFCカードの活用やシステムへの簡単なチェック入力で完了できるようにし、複雑な算定業務や請求手続きを抜本的に効率化します。
    *   これにより、事務作業の負担とミスを削減し、効率的で透明性の高い介護保険制度を実現します。
