...
### （玄人向け）YAML形式による政策提案の活用法

より構造化された明確なご提案をいただくために、YAML形式の活用を推奨します。これは、AIが提案内容を正確に理解し、より質の高い議論につなげるために非常に有効です。

**ステップ・バイ・ステップ ガイド:**

1.  **政策アイデアの構造化**: 毎日の政策メモや発言原稿案などを、YAML形式に整理します。「主張」「根拠」「対象」「財源」といった項目で構造化することで、AIが内容を正確に理解しやすくなります。
2.  **AIへの入力**: 整理したYAML形式のテキストを、いどばたシステムのチャットなどにコピー＆ペーストし、AIとの対話を開始します。原稿や答弁、キャッチフレーズの案などが、より明確に出力されることを確認できます。
3.  **複雑な課題への挑戦**: 慣れてきたら、市民対話の内容整理、法案要約、対立意見の比較、選挙公約の草案作成など、より複雑なプロンプトに挑戦しましょう。
4.  **提案精度の向上**: AIは、プロンプトの「構造」と「文脈」に大きく影響されます。YAMLを活用して意図や背景情報を明示することで、AIによる分析や提案の精度が飛躍的に高まります。
5.  **応用（ビジュアルの活用）**: 地域課題のビジュアル提示（マップやポスターなど）が必要な場合、画像をAIに読み込ませ、「このマップの要素をYAML化して」と指示することで、政策のビジュアル化を効率的に進める手助けになります。
6.  **カスタマイズと応用**: YAMLで構造化した政策プロンプトの一部（例：地域名、対象年齢層、費用試算など）を書き換えるだけで、様々なパターンの比較案を容易に生成できます。選挙戦略の立案や議会答弁の準備にも有効です。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキキップできます。
