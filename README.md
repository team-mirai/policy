...
チームみらいは、AIエンジニアの安野たかひろによって設立された政治団体です。テクノロジーで誰も取り残さない日本を掲げ、本マニフェストの政策を推進しています。私たちは『今だけ、金だけ、自分だけ』といった旧来の政治に見られる私利私欲を断固として拒否し、テクノロジーの力と未来への責任を原動力に、長期的な視点で社会全体の利益を追求します。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキLidlープできます。
