...
* **デジタル化推進におけるセーフティネットの原則**：デジタル化の恩恵を誰もが享受できる社会を目指す一方で、デジタル機器の利用が困難な方や、災害・通信障害時など、オフラインでの対応が不可欠な状況も想定します。そのため、デジタル化と並行して、代替手段やセーフティネットを必ず確保します。また、マイナ保険証を持たない方の選択を尊重し、従来の保険証も引き続き利用できる資格確認の方法を維持し、それにより不利益が生じることのないようにします。
