...
あんりさんのご提案に基づき、動物愛護に関する以下の政策を追加します。

* **ブリーダーの許可制の導入**: 現行の登録制から、より厳格な基準を設けた許可制へと移行し、ブリーダーの質を担保します。これにより、動物たちが心身ともに健康に生まれ育つ環境を整備します。
* **動物虐待の基準の明確化と厳罰化**: 何が動物虐待にあたるのか、その基準をより具体的に定め、社会全体で認識を共有します。加えて、虐待行為に対する罰則を強化し、動物たちの命と安全を断固として守ります。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキestepできます。
