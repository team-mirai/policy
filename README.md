...
1.  **一つの政策には、一つの役割を（単一責任の原則）**：例えば、「子育て支援」という政策は、子育て世帯の経済的負担軽減に集中させ、所得の再分配など他の目的とは切り分けます。これにより、政策の意図が明確になります。

2.  **政策同士をシンプルにつなげる（疎結合）**：ある政策の変更が、他の政策に複雑な影響を与えないようにします。例えば、子育て支援の給付額を、所得税の複雑な計算に頼らず、「子供の数」など単純な条件で決めます。

3.  **同じ目的の制度は一つにまとめる（DRY原則）**：似たような目的の給付金や手続きが複数あると複雑です。これらを一つに統合し、シンプルにします。
