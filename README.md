...
* **デジタル化における利用者への配慮の徹底**：デジタル化を進める際には、スマートフォンなどの操作が苦手な方でも不便がないよう、必要に応じて従来の物理的な手段（例：紙の書類）も選択できるなど、利用者の状況に合わせた丁寧な対応を心がけます。
* **個人情報の厳格な管理**：デジタルサービスで取り扱う個人情報は、本人の同意に基づき、必要な権限を持つ担当者のみが業務上必要な場合に限り閲覧できるなど、プライバシー保護を徹底します。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキ_notification` ラベルを付けることで通知をスキップできます。
