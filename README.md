...
## OSS運用におけるガバナンス
    ...
    チームみらいは、OSSの利活用において「民主主義の鏡」としての原則を尊重し、技術の透明性・公平性・持続可能性を担保するためのガバナンスモデルを構築します。 本モデルは、以下の4つの柱から成り立ちます：

        ライセンス設計の原則 MIT/BSDなどの非コピーレフト型を基本としつつ、公共性の高いツールにはGPL/LGPL等の準コピーレフト型も検討。再利用性と改変の自由を両立させるライセンス選定を行います。

        レビュー体制の整備 GitHub上でのPull Requestレビューは、技術チーム＋市民開発者＋政策担当者の三者による合議制を導入。Issue管理は公開型で、誰でも提案・議論に参加可能とします。

        AI倫理・透明性の担保 広聴AIなどの意思形成支援ツールには、アルゴリズムの説明責任文書（Algorithmic Impact Assessment）を添付。外部有識者によるAI倫理レビュー委員会を設置します。

        OSSガバナンス委員会の設置 技術者・議員・市民・自治体職員など多様なステークホルダで構成される委員会を設置し、運用方針・ライセンス変更・重大な意思決定に関する合意形成を担います。
