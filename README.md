...
インバウンド需要の取り込みと国内産業のブランド価値向上のため、政府が認定する新たな品質認証制度を創設します。和菓子、食品、嗜好品、料理店、宿泊施設、工芸品、家電、日用品と、多岐にわたる分野を対象とします。

認定基準には、**国内に本社があること**、**従業員の半数以上が日本人であること**のほか、製品・サービスの品質や環境への配慮、そして「就労環境の良さ」を重要な柱とします。この「就労環境の良さ」の重視は、後継者不足に悩む優れた技術を持つ中小企業の事業承継を後押しする狙いもあります。就労環境の評価は、公的データや民間調査会社の情報を活用し、賃金、労働時間、休暇取得率などを客観的に判断する仕組みを構築します。集約した就労環境データは、求職者にも提供し、就職のミスマッチ解消を目指します。また、国内材料の使用、障害者を雇用する、被災地や公的機関への寄付といった社会貢献も評価の対象とします。

認定された事業者は、「エンペラー・ワラント・マーク保持者協会」に加入します。協会は、登録料（分割または一括納付）を財源とし、マークの不正使用の取り締まり、品質管理活動、審査基準の維持・設定を行います。マークにはメインマークの周りか下に貢献度を示す等級と登録開始年数を記入し、品質保持と技術継承への努力を可視化します。登録が取り消された場合は、再度初めの等級からやり直しとなり、品質を維持するインセンティブとします。
