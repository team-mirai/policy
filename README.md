...
## 多様な家族のあり方の尊重（同性婚の法制化）

私たちは、誰もが愛する人と法的に家族となる選択ができる社会を目指します。性的指向にかかわらず、全ての人が平等に結婚する権利を保障するため、同性婚を法制化します。これにより、社会保障や相続など、婚姻に伴う権利と責任を同性カップルにも平等に適用し、個人の尊厳を守ります。
...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキG`skip-notification` ラベルを付けることで通知をスキップできます。
