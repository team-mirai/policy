...
　同様に、私たちは制度の公平性を重んじます。国民健康保険をはじめとする社会保障制度は、加入者全員が公平に支え合うことで成り立っています。そのため、保険料の未払いや不正な給付申請など、制度の信頼を損なう行為に対しては、国籍を問わず、やむを得ない事情がある場合を除き、厳正に対処する仕組みを構築し、制度の持続可能性を確保します。さらに、公平な制度運営と並行して、医療費の最適化も進めます。デジタル技術を最大限に活用し、診療データを分析することで、過剰な投薬や重複検査などを抑制し、医療の質の向上と制度の持続可能性を両立させます。
