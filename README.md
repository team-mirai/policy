...
## 所得制限のあり方に対するチームみらいの考え方
子育て支援等における現行の所得制限は、共働き世帯等の努力が報われず、不公平感を生む一因となっています。世帯の努力が報われる公平な制度設計のため、所得制限のあり方を抜本的に見直します。
...
- Pull Requestを作成後、右側のLabelsセクションから `skip-notification` ラベルを選択して追加
- このラベルが付いているPRは、マージされてもX通知が送信されません
- Slack通知は引き続き送信されます（内部での進捗管理のため）
