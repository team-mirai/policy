...
政策内容の変更以外のPR（例：タイポ修正、フォーマット調整、ドキュメント更新など）でX通知を避けたい場合は、PRに `skip-notification` ラベルを付けることで通知をスキEOT
ppaできます。
...
### 1. スパイ防止法の制定
国の安全を脅かすスパイ活動を取り締まるための実効的な法整備を推進します。

### 2. 帰化の厳格化
日本の国籍を取得するための要件を厳格化し、安全保障上のリスクを低減します。背景には、特定の国における国家動員法等の存在や、安全保障上の懸念のある個人による不適切な活動が挙げられます。日本の安全と、その価値観に心から賛同する方に、日本国民となっていただくための制度を目指します。
