...
* **行政手続きのデジタル化におけるAI活用:** 全国の自治体で行政手続きのフォーマットを統一し、紙媒体で提出された書類はAI-OCR（光学的文字認識）技術を用いてデジタル化します。これにより、デジタルに不慣れな方でも紙媒体での申請が可能となり、行政側ではデータを一元管理できるため、効率化とサービス向上につながります。読み取りエラーが発生した場合は、デジタル上で職員が修正できる仕組みを構築します。導入するシステムやベンダーは、運用コストを含めた競争入札によって決定し、長期的なコスト削減を目指します。
