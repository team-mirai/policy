...
def update_next2_100_results():
    """次の100件分析（第2弾）の結果をreadme-pr.yamlに反映"""

    # バッチ1とバッチ2の結果を読み込み
    with open('next2_100_batch1_results.yaml', 'r', encoding='utf-8') as f:
        batch1_data = yaml.safe_load(f)

    with open('next2_100_batch2_results.yaml', 'r', encoding='utf-8') as f:
    ...
    with open('next2_100_update_info.yaml', 'w', encoding='utf-8') as f:
        yaml.dump(update_info, f, default_flow_style=False, allow_unicode=True)

    print(f"\n更新情報を next2_100_update_info.yaml に保存しました")
