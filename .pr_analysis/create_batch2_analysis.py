...
def create_batch_analysis_file(pr_numbers):
    ...
    content = f"""# バッチ分析2 - PR #{pr_numbers[0]} から #{pr_numbers[-1]}
...
### PR本文
{pr_info['body'][:1500] if pr_info['body'] else '（本文なし）'}{'...(省略)' if pr_info['body'] and len(pr_info['body']) > 1500 else ''}

### 差分内容
```diff
{pr_info['diff'][:1500]}{'...(省略)' if len(pr_info['diff']) > 1500 else ''}
```
...
"""
    ...
    filename = f"batch_analysis2_{pr_numbers[0]}_{pr_numbers[-1]}.md"
...
if __name__ == "__main__":
    pr_numbers = [1856, 1854, 1851, 1849, 1844, 1843, 1841, 1840, 1834,
                  1831, 1826, 1822, 1820, 1818, 1814, 1813, 1809, 1808, 1801, 1791]

    print(f"バッチ分析ファイル2を作成中... ({len(pr_numbers)}件)")
