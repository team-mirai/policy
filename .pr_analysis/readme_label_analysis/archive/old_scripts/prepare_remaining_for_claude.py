...
def main():
    ...
    # バッチ1は完了したので、51件目以降を処理対象とする
    remaining_prs = all_unclassified[50:]  # 51件目から
    
    print(f"残り処理対象: {len(remaining_prs)}件")
    
    # 新しい未分類リストを作成
    remaining_file = "remaining_259_prs_for_claude.csv"
    with open(remaining_file, 'w', encoding='utf-8-sig', newline='') as f:
    ...
    print(f"✓ 残り259件のリスト作成: {remaining_file}")
    
    # 実行用スクリプトの調整版を作成
    script_content = '''#!/usr/bin/env python3
"""
残り259件のPRをclaude -pで分類
"""
import csv
import subprocess
import json
...
def main():
    ...
    if not os.path.exists(remaining_file):
        print(f"エラー: {remaining_file} が見つかりません")
        return
    
    remaining_prs = []
    with open(remaining_file, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        remaining_prs = list(reader)
    
    print(f"残り未分類PR数: {len(remaining_prs)}件")
    print("分類を開始します...\\n")
    ...
    for i, pr in enumerate(remaining_prs):
        ...
        # 進捗表示
        progress = (i + 1) / len(remaining_prs) * 100
        elapsed = time.time() - start_time
        eta = elapsed / (i + 1) * len(remaining_prs) - elapsed if i > 0 else 0
        
        print(f"[{i+1}/{len(remaining_prs)}] ({progress:.1f}%) PR #{pr_num}: {pr_title[:50]}...")
        ...
        results.append({
            ...
            '分析メモ': f'claude-p分類: {datetime.now().strftime("%Y-%m-%d")}',
            'URL': pr['URL'],
            '説明（200文字まで）': pr_title[:200]
        })
        ...
        # 10件ごとに中間保存
        if (i + 1) % 10 == 0:
            temp_file = f'temp_classified_{i+1}.csv'
    ...
    # 最終結果を保存
    output_file = f'claude_classified_259_prs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
'''
    ...
    with open('claude_classify_remaining.py', 'w', encoding='utf-8') as f:
        f.write(script_content)
    
    print(f"✓ 実行用スクリプト作成: claude_classify_remaining.py")
    
    print("\n=== 実行方法 ===")
    print("1. 新しいターミナルを開く")
    print("2. このディレクトリに移動")
    print("3. 以下を実行:")
    print("   python3 claude_classify_remaining.py")
    print("")
    print("完了したら、統合用スクリプトを実行します。")
