#!/usr/bin/env python3
"""
バッチ3の分類結果をreadme-pr-merged.yamlに適用するスクリプト
"""
...
def apply_batch3_classifications():
    # 分類結果を読み込む
    classifications = load_yaml('batch3_classification_results.yaml')
    ...
    # 分類結果を適用
    updated_count = 0
    for classification in classifications['batch_3_classifications']:
        ...
        for pr in data['pull_requests']:
            if pr['number'] == pr_number:
                ...
                pr['analysis_notes'] = f"LLM分析バッチ3: {datetime.now().strftime('%Y-%m-%d')}"
                updated_count += 1
                break
    ...
    print(f"  進捗率: {total_classified / len(data['pull_requests']) * 100:.1f}%")

if __name__ == "__main__":
    apply_batch3_classifications()