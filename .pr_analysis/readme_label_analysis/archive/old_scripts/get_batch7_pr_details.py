...
# Load batch 7 data
with open('batch7_20250714_2249.yaml', 'r', encoding='utf-8') as f:
    batch_data = yaml.safe_load(f)

# Extract PR numbers
pr_numbers = [pr['number'] for pr in batch_data['pull_requests']]

print(f"バッチ7のPR詳細を取得します: {len(pr_numbers)}件")

# Create analysis file
analysis_filename = f"analysis_batch7_{datetime.now().strftime('%Y%m%d_%H%M')}.md"

with open(analysis_filename, 'w', encoding='utf-8') as f:
    f.write(f"# PR分析バッチ7 - {datetime.now().strftime('%Y年%m月%d日 %H:%M')}\n\n")
