...
def get_next_batch2():
    ...
    # 次の35件を取得（30件スキップして35件取得）
    batch_prs = unanalyzed_prs[30:65]
    
    print(f"=== 次の100件分析 - バッチ2（35件） ===")
    ...
    # バッチ情報をファイルに保存
    batch_info = {
        'batch_name': 'next_100_batch2',
        'batch_size': len(batch_prs),
        'pr_range': f"#{batch_prs[-1]['number']}-#{batch_prs[0]['number']}",
        'prs': [pr['number'] for pr in batch_prs]
    }
    
    with open('next_100_batch2_info.yaml', 'w', encoding='utf-8') as f:
        yaml.dump(batch_info, f, default_flow_style=False, allow_unicode=True)
    
    print(f"\nバッチ情報を next_100_batch2_info.yaml に保存しました")
    print(f"次のステップ: create_next_100_batch2_analysis.py を実行してください")
