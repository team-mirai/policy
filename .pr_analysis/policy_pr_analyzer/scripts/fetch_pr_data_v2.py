#!/usr/bin/env python3
"""
GitHub APIを使用して指定されたラベルのPRデータを取得する（改良版）
"""
...
def get_pr_details(pr_number, cache_dir=None):
    ...
    args = [
        ...
        "--repo", repo,
        "--json", "number,title,body,author,createdAt,updatedAt,state,url,labels,reactions,additions,deletions,changedFiles,mergeable,merged,mergedAt"
    ]
    ...
    comments_result = run_gh_command(comments_args, check=False)
    if comments_result:
        try:
            comments_data = json.loads(comments_result)
            pr_data['commentsCount'] = len(comments_data.get('comments', []))
        except:
            pr_data['commentsCount'] = 0
    else:
        pr_data['commentsCount'] = 0
...
def main():
    parser = argparse.ArgumentParser(description='Fetch PR data from GitHub (改良版)')
