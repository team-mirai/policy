...
def get_next2_batch2():
    """次の100件分析（第2弾）のバッチ2（50件）を取得する"""
    ...
    with open('next2_100_batch2_info.yaml', 'w', encoding='utf-8') as f:
        yaml.dump(batch_info, f, default_flow_style=False, allow_unicode=True)

    print(f"\nバッチ情報を next2_100_batch2_info.yaml に保存しました")
    print(f"次のステップ: analyze_next2_100_batch2.py を実行してください")
