...
# Get unclassified PRs (excluding ones we've already done in batches 1-4)
# Based on the PR numbers from previous batches
...
classified_prs = set(batch1_prs + batch2_prs + batch3_prs + batch4_prs)
...
# Save batch 5
filename = f"batch5_{datetime.now().strftime('%Y%m%d_%H%M')}.yaml"
with open(filename, 'w', encoding='utf-8') as f:
    yaml.dump(batch5_data, f, allow_unicode=True, default_flow_style=False, sort_keys=False)

print(f"バッチ5を作成しました: {filename}")
