...
print(f"バッチ6のPR詳細を取得します: {len(pr_numbers)}件")

# Create analysis file
analysis_filename = f"analysis_batch6_{datetime.now().strftime('%Y%m%d_%H%M')}.md"

with open(analysis_filename, 'w', encoding='utf-8') as f:
    f.write(f"# PR分析バッチ6 - {datetime.now().strftime('%Y年%m月%d日 %H:%M')}\n\n")
