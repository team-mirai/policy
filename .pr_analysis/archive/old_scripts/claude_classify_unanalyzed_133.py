...
def main():
    # 未分析133件のPRを読み込み
    remaining_file = "unanalyzed_133_prs_for_claude.csv"
    ...
    print(f"未分析PR数: {len(remaining_prs)}件")
    print("分類を開始します...\n")
    ...
    for i, pr in enumerate(remaining_prs):
        ...
        # 10件ごとに中間保存
        if (i + 1) % 10 == 0:
            temp_file = f'temp_unanalyzed_{i+1}.csv'
    ...
    # 最終結果を保存
    output_file = f'claude_classified_unanalyzed_133_prs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
